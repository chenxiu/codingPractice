package practice;

public class PackProblem_DuplicateElement {

  public static void main(String[] args) {
    int n = 4;// number of elements
    int A[] = {7, 3, 2, 5}; // bulk of each element, duplicate element allows
    int V[] = {1, 5, 2, 4}; // value of each element
    int m = 11; // pack volume
    System.out.println(uniqueSteps(n, A, V, m));

  }

  private static int getMax(int a, int b) {
    if (a >= b)
      return a;
    else
      return b;
  }

  private static int uniqueSteps(int n, int[] A, int[] V, int m) {
    int[][] B = new int[n + 1][m + 1];
    for (int j = 0; j <= m; j++)
      B[0][j] = 0;
    for (int i = 1; i <= n; i++) {
      for (int j = 0; j <= m; j++) {
        B[i][j] = B[i - 1][j];
        if (j - A[i - 1] >= 0)
          B[i][j] = getMax(B[i - 1][j], B[i - 1][j - A[i - 1]] + V[i - 1]);
        // B[i-1][j] : if not put new staff A[i-1] inside
        // B[i][j-A[i-1]]+V[i-1] : if put new staff A[i-1] inside, value
        // increase by V[i-1]
      }
    }
    for (int i = 0; i <= n; i++) {
      for (int j = 0; j <= m; j++) {
        System.out.print(B[i][j] + " ");
      }
      System.out.println();
    }

    return B[n][m];
  }

}
